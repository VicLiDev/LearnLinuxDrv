...
对于只能访问32位地址空间（0-4GB）的设备，需要设置相应的DMA掩码。以下是几种常用方法：

#### 方法1：使用 dma_set_mask_and_coherent()（推荐）

这是最常用的方法，通过设置DMA掩码告诉内核设备的寻址能力：
```c
// 同时设置流式DMA和一致性DMA的掩码为32位
...
`DMA_BIT_MASK(32)` 表示设备只能访问低32位地址空间（0 ~ 4GB-1）。

#### 方法2：分别设置流式和一致性掩码

如果需要对流式DMA和一致性DMA使用不同的掩码，可以分别设置：

```c
// 设置流式DMA掩码
if (dma_set_mask(dev, DMA_BIT_MASK(32))) {
        dev_warn(dev, "mydev: 32-bit streaming DMA not available\n");
        return -EIO;
}

// 设置一致性DMA掩码
if (dma_set_coherent_mask(dev, DMA_BIT_MASK(32))) {
        dev_warn(dev, "mydev: 32-bit coherent DMA not available\n");
        return -EIO;
}
```
...
对于使用 `kmalloc()` 或 `__get_free_pages()` 直接分配的内存，可以使用 GFP 标志限制物理地址范围：

```c
// GFP_DMA32: 保证分配的物理地址在低32位（0-4GB）范围内
void *buf = kmalloc(size, GFP_DMA32 | GFP_KERNEL);

// GFP_DMA: 保证分配的物理地址在低24位（0-16MB）范围内
// 适用于老式ISA设备
void *buf = kmalloc(size, GFP_DMA | GFP_KERNEL);
```

**GFP 标志说明：**
| 标志        | 地址范围      | 适用场景    |
| ----------- | ------------- | ----------- |
| `GFP_DMA`   | 0-16MB (24位) | 老式ISA设备 |
| `GFP_DMA32` | 0-4GB (32位)  | 32位PCI设备 |
| 无标志      | 任意地址      | 64位设备    |

#### 方法4：DMA映射时的自动限制

当使用DMA API映射内存时，内核会根据之前设置的 `dma_mask` 自动限制地址范围：
...
static int my_device_probe(struct platform_device *pdev)
{
        ...
        // 之后所有使用DMA API的分配和映射都会自动限制在4G以下
        // ...

        return 0;
}

// 使用DMA API时，内核自动保证返回的地址在4G以下
...
static int my_driver_probe(struct platform_device *pdev)
{
        ...
        /* 方法2：尝试64位，失败则回退到32位 */
        if (dma_set_mask_and_coherent(dev, DMA_BIT_MASK(64))) {
                dev_warn(dev, "64-bit DMA not available, using 32-bit\n");
                if (dma_set_mask_and_coherent(dev, DMA_BIT_MASK(32))) {
                        dev_err(dev, "32-bit DMA also not available\n");
                        return -EIO;
                }
        }

        /* DMA映射时会自动使用设置的掩码 */
        cpu_addr = kzalloc(size, GFP_KERNEL);
        if (!cpu_addr)
                return -ENOMEM;
        ...
        dev_info(dev, "DMA address: %pad (guaranteed < 4GB)\n", &dma_handle);
...
1. **必须在DMA操作前设置掩码**：在调用任何DMA API之前，必须先调用 `dma_set_mask()`
   或 `dma_set_mask_and_coherent()`。
2. **检查返回值**：设置DMA掩码可能失败，特别是在不支持指定地址范围的系统上，必须
   检查返回值。
3. **掩码一致性**：一致性DMA掩码应该设置成与流式DMA掩码相同或更小的值。
4. **GFP标志的局限性**：`GFP_DMA32` 等标志主要用于底层内存分配，使用DMA API时不
   需要（也不应该）依赖这些标志，而是通过设置 `dma_mask` 来控制。

总结：对于需要限制物理地址在4G以下的设备，推荐的做法是在驱动 probe 函数中调用
`dma_set_mask_and_coherent(dev, DMA_BIT_MASK(32))`，这样之后所有使用DMA API的
分配和映射都会自动限制在4G以下。
...
##### 使用 sg_table 结构（现代 API）

现代内核（v4.6+）推荐使用 `struct sg_table` 代替直接操作 `struct scatterlist` 数组。
`sg_table` 将 scatterlist 分为两类：
- `sgl` / `orig_nents`：CPU 端看到的段数（原始段数）
- `sgl` / `nents`：设备/DMA 端看到的段数（可能被 IOMMU 合并）
...
for_each_sgtable_sg(sgt, sg, i) {
    ...
    pr_info("sg[%d]: dma_addr=0x%pad, phys=0x%pa, len=0x%x\n",
...
**调试打印示例（你的代码风格）：**
```c
/* Print physical address for debugging */
if (buffer->sgt && buffer->sgt->sgl) {
    struct scatterlist *sg;
    int i;

    for_each_sgtable_sg(buffer->sgt, sg, i) {
        phys_addr_t phys_addr = sg_phys(sg);
        dma_addr_t dma_addr = sg_dma_address(sg);
        unsigned int sg_len = sg_dma_len(sg);

        pr_info("fd=%d, sg[%d]: phys_addr=0x%pa (below 4G: %s), dma_addr/iova=0x%pad, len=0x%x\n",
               fd, i, &phys_addr, phys_addr < SZ_4G ? "yes" : "no",
               &dma_addr, sg_len);
    }
}
```
...
// 或者如果是 dma_alloc_noncontiguous 分配的
...
| 旧 API                           | 新 API                            | 说明             |
|----------------------------------|-----------------------------------|------------------|
| `for_each_sg(sgl, sg, nents, i)` | `for_each_sgtable_sg(sgt, sg, i)` | 遍历 scatterlist |
| `dma_map_sg()`                   | `dma_map_sgtable()`               | DMA 映射         |
| `dma_unmap_sg()`                 | `dma_unmap_sgtable()`             | DMA 取消映射     |
| `sg_init_table()`                | `sg_alloc_table()`                | 初始化 sg_table  |
