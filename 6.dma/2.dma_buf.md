...
对该buffer再做个字节序交换，然后才送给SPI DMA，待DMA传输完成后执行dma_unmap_single()操作。
因此这里也出现了dma map和unmap期间CPU要访问这段内存的需求。
...
CPU 访问内存之前，通过调用`dma_sync_{single,sg}_for_cpu()`来Invalidate Cache，这样CPU
