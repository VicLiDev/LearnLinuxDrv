...
{
    pr_info("work_struct_demo: work_handler running in process: %s\n", current->comm);
    msleep(300); // Simulate workload
    pr_info("work_struct_demo: work_handler done\n");
}

static void cancelable_handler(struct work_struct *work)
{
    pr_info("work_struct_demo: cancelable_handler started\n");
    msleep(500); // Long-running work
    pr_info("work_struct_demo: cancelable_handler completed\n");
}

static void delayed_handler(struct work_struct *work)
{
    pr_info("work_struct_demo: delayed_handler triggered after delay\n");
}
...
static int m_chrdev_open(struct inode *inode, struct file *file)
{
    ...
    pr_info("work_struct_demo: module loaded\n");
    ...
    // 2. 可取消的 work
    INIT_WORK(&cancelable_work, cancelable_handler);
    schedule_work(&cancelable_work);

    // 3. 延迟 work
    INIT_DELAYED_WORK(&my_delayed_work, delayed_handler);
    schedule_delayed_work(&my_delayed_work, msecs_to_jiffies(1000)); // 1 秒后执行
...
static int m_chrdev_release(struct inode *inode, struct file *file)
{
    ...
    pr_info("work_struct_demo: module unloading\n");

    // 同步取消 cancelable work（如果它还没开始或正在运行）
    canceled = cancel_work_sync(&cancelable_work);
    if (canceled)
        pr_info("work_struct_demo: cancelable_work was running and canceled\n");
    else
        pr_info("work_struct_demo: cancelable_work had already finished\n");

    // 同步取消 delayed work（如果还没执行）
    canceled = cancel_delayed_work_sync(&my_delayed_work);
    if (canceled)
        pr_info("work_struct_demo: delayed_work canceled before execution\n");
    else
        pr_info("work_struct_demo: delayed_work already executed\n");

    // 等待所有 work_struct 完成
    // flush_scheduled_work();
    // 但要避免 flush_scheduled_work()
    // 1. 它会阻塞当前进程，直到所有提交到 system_wq 的任务都完成。
    //    哪怕是别的模块、驱动提交的工作也会等待。
    // 2. 影响范围过大：
    //    系统级的工作队列可能有上百个任务，flush 会带来不确定性。
    // 3. 容易死锁或产生延迟：
    //    如果你在一个 work handler 里再调用它，有可能形成自等待死锁。
    // 4. 可替代方案更安全。
    //
    // 推荐替代方案
    // 使用 flush_work() 或 flush_delayed_work() 对单个任务进行同步：
    //   flush_work(&my_work);                   // 等待 my_work 完成
    //   flush_delayed_work(&my_delayed_work);   // 等待延迟工作完成
    // 优点：只等待你关心的工作项，安全，粒度精细。
    //
    flush_work(&my_work);
    flush_work(&cancelable_work);
    flush_delayed_work(&my_delayed_work);


    pr_info("work_struct_demo: module exited cleanly\n");
