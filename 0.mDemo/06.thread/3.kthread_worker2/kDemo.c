...
static struct kthread_work cancelable_work;

static void work_func(struct kthread_work *work)
{
    pr_info("kthread_worker_demo: [work_func] running in thread: %s\n", current->comm);
    msleep(100);
    pr_info("kthread_worker_demo: [work_func] done\n");
}

static void delayed_work_func(struct kthread_work *work)
{
    pr_info("kthread_worker_demo: [delayed_work_func] executed after delay\n");
}

static void cancelable_work_func(struct kthread_work *work)
{
    pr_info("kthread_worker_demo: [cancelable_work_func] running...\n");
    msleep(500);
    pr_info("kthread_worker_demo: [cancelable_work_func] finished\n");
}

static int worker_thread_fn(void *data)
{
    struct kthread_worker *worker = data;
    return kthread_worker_fn(worker);
}
...
static int m_chrdev_open(struct inode *inode, struct file *file)
{
    ...
    pr_info("kthread_worker_demo: module init\n");

    // 初始化 worker 对象
    kthread_init_worker(&my_worker);

    // 创建并运行线程
    worker_thread = kthread_run(worker_thread_fn, &my_worker, "kthread_demo_worker");
    if (IS_ERR(worker_thread)) {
        pr_err("kthread_worker_demo: Failed to create kthread\n");
    ...
    kthread_queue_delayed_work(&my_worker, &my_delayed_work, msecs_to_jiffies(1500));

    // 3. 可取消工作项
    kthread_init_work(&cancelable_work, cancelable_work_func);
    kthread_queue_work(&my_worker, &cancelable_work);

    // 尝试取消它（在 exit 再实际同步取消）
...
static int m_chrdev_release(struct inode *inode, struct file *file)
{
    ...
    pr_info("kthread_worker_demo: module exit\n");

    // 尝试取消 cancelable work
    cancelled = kthread_cancel_work_sync(&cancelable_work);
    if (cancelled)
        pr_info("kthread_worker_demo: cancelable_work was canceled before completion\n");
    else
        pr_info("kthread_worker_demo: cancelable_work had already finished or not started\n");

    // 尝试取消延迟工作项
    cancelled = kthread_cancel_delayed_work_sync(&my_delayed_work);
    if (cancelled)
        pr_info("kthread_worker_demo: delayed_work was canceled before execution\n");
    else
        pr_info("kthread_worker_demo: delayed_work had already run or not scheduled\n");

    // 等待所有 work 执行完毕
    kthread_flush_worker(&my_worker);

    // 停止线程
    kthread_stop(worker_thread);

    pr_info("kthread_worker_demo: cleanup complete\n");
