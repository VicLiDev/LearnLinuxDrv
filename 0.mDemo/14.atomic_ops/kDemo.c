...
static atomic_t bitflags = ATOMIC_INIT(0b1100);
...
static void arithmetic_demo(void)
{
    ...
    atomic_sub(3, &counter);   // 减 3
    atomic_inc(&counter);      // 加 1
    atomic_dec(&counter);      // 减 1

    printk(KERN_INFO "Arithmetic Demo: counter = %d\n", atomic_read(&counter));
}

// ====== 原子按位操作演示 ======
static void bitwise_demo(void)
{
    atomic_or(0b0011, &bitflags);   // OR 操作
    atomic_and(0b1110, &bitflags);  // AND 操作
    atomic_xor(0b0101, &bitflags);  // XOR 操作

    printk(KERN_INFO "Bitwise Demo: bitflags = 0b%04x\n", atomic_read(&bitflags));
}

// ====== 原子比较并交换（CAS） ======
static void cas_demo(void)
{
    int expected = 0;
    int new_value = 100;

    // 若 *v == expected，则设置为 new_val，返回旧值
    // 若 *v != expected，*v 不变，返回原来的值（即旧值 *v），不执行赋值操作
    int old = atomic_cmpxchg(&counter, expected, new_value);
    if (old == expected)
        printk(KERN_INFO "CAS Success: counter = %d\n", atomic_read(&counter));
    else
        printk(KERN_INFO "CAS Failed: counter = %d (expected %d, found %d)\n",
               atomic_read(&counter), expected, old);
}

// ====== 自旋锁演示（基于 CAS 实现） ======
static void spinlock_demo(void)
{
    // 简易自旋锁
    // 尝试将 lock 从 0 设为 1，失败则继续尝试（CAS）
    while (atomic_cmpxchg(&spinlock, 0, 1) != 0) {
        cpu_relax(); // 节省 CPU，等待锁
    }

    printk(KERN_INFO "Spinlock acquired in critical section\n");
...
// ====== 内存屏障演示 ======
static void memory_barrier_demo(void)
{
    int a = 0, b = 0;

    smp_mb(); // 完全屏障：所有 CPU 上的读写都不能跨越这个屏障

    a = 1;
    smp_wmb(); // 写屏障：保证写操作顺序
    b = 2;

    printk(KERN_INFO "Memory barrier example: a=%d, b=%d\n", a, b);
}
/*
 * ==============================
 * demo end
