...
内核提供了多种内存分配方式，按不同维度分类如下。DMA 相关的详细内容见
`07.dma/1.dma.md`，此处做概览。

## 按物理连续性分类

| 类型            | 虚拟连续 | 物理连续 | 典型 API                              | 适用场景                    |
|-----------------|----------|----------|---------------------------------------|-----------------------------|
| kmalloc/kzalloc | 是       | 是       | `kmalloc()`, `kzalloc()`              | 小块内存，通用内核对象      |
| vmalloc/vzalloc | 是       | 否       | `vmalloc()`, `vzalloc()`              | 大块内存，对性能不敏感      |
| 页分配          | 是       | 是       | `alloc_pages()`, `__get_free_pages()` | 以页为单位的分配            |
| SLAB/SLUB       | 是       | 是       | `kmem_cache_alloc()`                  | 频繁创建/销毁的固定大小对象 |

对应 demo: `00.mDemo/02.base/6.alloc_buf/kDemo.c`
...
```c
void *dma_alloc_coherent(dev, size, &dma_handle, gfp);
void  dma_free_coherent(dev, size, cpu_addr, dma_handle);
```
* 物理连续，CPU 和设备同时访问不需要手动 sync
* 内部实现通常是 uncacheable 映射，或依赖硬件 cache coherent interconnect
* 适合网卡描述符、SCSI 命令队列等长期存在的共享缓冲区

### 流式 DMA 映射 (Streaming DMA)

```c
dma_addr_t dma_map_single(dev, addr, size, direction);
void       dma_unmap_single(dev, dma_handle, size, direction);
```

* 映射已有内存（如 kmalloc 分配的），用完即解除映射
* 需要 CPU 和设备之间手动 sync (`dma_sync_single_for_cpu/device`)
* 适合网络 skb、文件系统 buffer 等短生命周期数据

### 散射聚集 DMA (Scatter-Gather DMA)

```c
int  dma_map_sg(dev, sglist, nents, direction);
void dma_unmap_sg(dev, sglist, nents, direction);
```

* 物理可以不连续，IOMMU 或设备 SG 能力将其视为连续
* 现代内核推荐 `sg_table` + `dma_map_sgtable()`

### 非一致性 DMA 分配

```c
struct page *dma_alloc_pages(dev, size, &dma_handle, dir, gfp);
void *dma_alloc_noncoherent(dev, size, &dma_handle, dir, gfp);
struct sg_table *dma_alloc_noncontiguous(dev, size, dir, gfp, 0);
```

* 分配 DMA 寻址的页面，但不做 cache 一致性保证
* 需要调用者自己管理 `dma_sync_*` 来维护一致性
* `dma_alloc_noncontiguous` 物理可以不连续

### DMA Pool

```c
struct dma_pool *dma_pool_create(name, dev, size, align, boundary);
void *dma_pool_alloc(pool, flags, &dma_handle);
void  dma_pool_free(pool, cpu_addr, dma_handle);
```

* 从 coherent 内存中切分小块，类似 `kmem_cache` 的 DMA 版本
* 适合大量小尺寸 DMA buffer（如 USB 传输描述符）

### DMA 内存选择指南

| 场景 | 推荐方式 |
|---------------------|---------|
| 设备长期共享 buffer | `dma_alloc_coherent()` (底层走 CMA) |
| 设备短期数据传输    | `dma_map_single()` + sync |
| 大块非连续 DMA      | `dma_alloc_noncontiguous()` + `sg_table` |
| 大块连续物理内存 (摄像头/显示) | CMA (通过 `dma_alloc_contiguous`) |
| 大量小 DMA 描述符 | `dma_pool` |

## CMA (Contiguous Memory Allocator)

CMA 横跨在通用内存和 DMA 内存之间，是内核为了解决"大块连续物理内存难分配"问题
而设计的机制。

### 核心思想

在启动时预留一块大的物理连续区域。平时这些页面可以被普通的 `kmalloc`/`alloc_pages`
使用（作为可移动页面），当设备驱动需要大块连续物理内存时，内核通过**页面迁移
(migration)** 把普通用户挤走，腾出连续空间给 DMA 使用。
...
```
     ...
     |           |  页面给 DMA 使用  |           |
     +-----------+                  +-----------+
       kmalloc等      migrate           dma_alloc_coherent
       临时借用                         /alloc_pages 等长期占用
```

### 关键 API

驱动一般不直接调用 CMA API，而是通过以下接口间接受益。当 CMA 可用时，
`dma_alloc_coherent()` 会优先从 CMA 区域分配，这样更容易获得大块连续物理内存。

```c
// 驱动侧: 通过 dma_alloc_coherent 间接受益
cpu_addr = dma_alloc_coherent(dev, size, &dma_handle, GFP_KERNEL);

// 底层: 内核内部调用 dma_alloc_contiguous 从 CMA 分配
struct page *dma_alloc_contiguous(struct device *dev, size_t size, gfp_t gfp);
```

### CMA 配置

通过内核启动参数或 device tree 配置：

```
# 内核启动参数
cma=64M          # 预留 64MB CMA 区域
cma=256M@512M    # 从 512M 物理地址处预留 256MB
cma=0            # 禁用 CMA
```

通过 /proc/meminfo 查看 CMA 状态：
```
CmaTotal:         65536 kB
CmaFree:          32768 kB
```
...
* 优点: 不用时内存不浪费，普通分配也能用；保证大块连续分配成功
* 缺点: 分配时需要迁移页面，有延迟；迁移失败则分配失败
...
| (旧ISA) | (32位设备)|   (常规内存)     | (可迁移页面)|
+---------+-----------+------------------+-------------+
                                        ^
                                   CMA 页面来自这里
```

CMA 页面属于 `ZONE_MOVABLE` 类型，这意味着它们始终可以被迁移。当 CMA 需要分配
时，内核会把属于该区域的可移动页面迁移到其他位置，腾出连续空间。
...
| 类型                  | 说明                             | 典型用途               |
|-----------------------|----------------------------------|------------------------|
| CMA                   | 可被普通分配临时借用，需要时回收 | 摄像头、显示、大块 DMA |
| memblock reserved     | 启动时永久预留，不可被内核分配   | 系统关键数据、启动日志 |
| reserved-memory (DT)  | device tree 中声明，驱动独占     | 固件、TPM、特定设备    |
| 内核 .init 段         | `__initdata` 等，启动后释放      | 内核初始化代码         |

### device tree 中的 reserved-memory 示例
...
* `no-map`: 该区域完全从内核内存管理中排除，驱动通过 `memremap()` 或
  `ioremap()` 访问
* `reusable`: 该区域可以被 CMA 管理，平时可被普通分配使用

### 驱动中引用 reserved-memory

设备节点通过 `memory-region` 属性引用 reserved-memory 区域：

```dts
/* reserved-memory 中声明 */
my_reserved: mybuf@0x50000000 {
...
static int my_probe(struct platform_device *pdev)
{
    ...
    /* 方式1: 通过 of_reserved_mem 获取 */
    rmem = of_reserved_mem_lookup(pdev->dev.of_node, "memory-region");
    if (rmem) {
        paddr = rmem->base;
        size  = rmem->size;
...
| 类型            | API                         | 说明                                                |
|-----------------|-----------------------------|-----------------------------------------------------|
| 高端内存映射    | `kmap()`, `kmap_atomic()`   | 32位系统映射 >896MB 的物理页                        |
| IO 内存映射     | `ioremap()`, `ioremap_wc()` | 将设备 MMIO 寄存器映射到内核虚拟地址空间            |
| 写合并内存      | `dma_alloc_wc()`            | 关闭 cache 一致性但允许 write combining，提升写性能 |
| mmap 到用户空间 | `remap_pfn_range()`         | 将内核/设备内存映射到用户进程                       |
| 内核栈          | 每进程 1-2 页               | 中断上下文栈，大小有限                              |
...
```
          ...
          +------+------+         +------+------+
          |             |         |             |
      需要 DMA?      设备寄存器  小块内存?     大块内存
          |             |         |             |
     +----+----+    ioremap()  kmalloc()    vmalloc()
     |         |                  kzalloc()
 连续物理?  非连续也行              kmem_cache
     |         |
 dma_alloc   dma_alloc
 _coherent   _noncontiguous
     |
     +---> 底层走 CMA 分配
```
...
| 分配方式                | 大小上限               | 物理连续 | 可用于 DMA   |
|-------------------------|------------------------|----------|--------------|
| kmalloc                 | ~128KB (架构相关)      | 是       | 是           |
| vmalloc                 | 接近可用虚拟地址空间   | 否       | 通常不可     |
| alloc_pages (order n)   | 2^n * PAGE_SIZE        | 是       | 是           |
| dma_alloc_coherent      | CMA 大小               | 是       | 是           |
| dma_alloc_noncontiguous | 更大                   | 否       | 是 (通过 SG) |
| kmem_cache              | slab 大小 (通常 128KB) | 是       | 是           |
