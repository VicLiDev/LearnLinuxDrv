 ...
 * | 宏            | 运行时条件是否为真？   | 编译器如何预测？                    |
 * | ------------- | ---------------------- | ----------------------------------- |
 * | `likely(x)`   | 如果 x 为真就进 if     | 编译器假设 **常为真**，优化真分支   |
 * | `unlikely(x)` | 如果 x 为真就进 if     | 编译器假设 **很少为真**，优化假分支 |
 *
 * likely(x) 和 unlikely(x) 并不控制条件本身的真假，而是告诉编译器：我认为这个
 * 条件在运行时会“更有可能”为真还是为假，从而帮助优化 CPU 的分支预测，提高性能。
 *
 * 用法建议
 * likely(x)：
 *   对主流程使用。
 *   例如函数中检查状态是否有效（大部分应该有效）。
 * unlikely(x)：
 *   对错误处理、断言、异常分支使用。
 */
static int unlikely_demo_init(void)
{
    int x = 1;
    int error = 0;

    if (likely(x == 1)) {
        pr_info("branch_hint_demo: likely path taken (x == 1)\n");
    } else {
        pr_info("branch_hint_demo: unlikely path (x != 1)\n");
    }

    // 模拟错误发生
    error = 1;
    if (unlikely(error)) {
        pr_warn("branch_hint_demo: unlikely error path triggered\n");
    }
